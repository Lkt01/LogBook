...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dbRef = FirebaseDatabase.getInstance().getReference("LogbookImage");

        binding.imageCamera.setOnClickListener(v -> CameraIntent());
        ...
        dbRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    Picasso.get().load(list.get(0).getImageURL()).into(binding.ivImage);
                    binding.tvPage.setText("1/" + list.size());
                    binding.tvAddress.setText(list.get(count).getAddress());
                    binding.etUrl.setText(list.get(count).getImageURL());
                    count = 0;
                }
            }
        ...
        binding.btnPrevious.setOnClickListener(v -> {
            if (count > 0) {
                count--;
                Picasso.get().load(list.get(count).getImageURL()).into(binding.ivImage);
                binding.tvPage.setText((count + 1) + "/" + list.size());
                binding.tvAddress.setText(list.get(count).getAddress());
                binding.etUrl.setText(list.get(count).getImageURL());
            }
        });
        ...
        binding.btnDownload.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                getLocation();
                ImageUploadInfo imageUploadInfo = new ImageUploadInfo(binding.tvAddress.getText().toString(), url);
                dbRef.push().setValue(imageUploadInfo);
    ...
    private boolean checkLink(String url) {
        //check url xem có phải la url hay không
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == REQUEST_CODE_CAMERA) {
            if (resultCode == RESULT_OK) {
                assert data != null;
                binding.imageCamera.setImageBitmap((Bitmap) data.getExtras().get("data"));
                getLocation();
                saveImage();
                uploadImg(bitmapToByte((Bitmap) data.getExtras().get("data")));
            }
        }
    }

    private void saveImage() {
        Bitmap bitmap = binding.imageCamera.getDrawingCache();
    ...
    private void uploadImg(byte[] image) {
        ...
        storageReference.putBytes(image)
                .addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                    ...
                    public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                        ...
                        ImageUploadInfo imageUploadInfo = new ImageUploadInfo(binding.tvAddress.getText().toString(), uploadImg);
